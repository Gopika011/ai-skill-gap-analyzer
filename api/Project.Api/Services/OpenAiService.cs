...
public class OpenAiService : IOpenAiService
{
    ...
    public async Task<string> AnalyzeSkillGapsAsync(string dataJson)
    {
        ...
        var systemInstruction = "You are an AI Skill Gap Analyzer. Analyze the project skill requirements against assigned employee skills. Group by employee, calculate gap severity (No Gap, Minor Gap, Moderate Gap, Critical Gap), and return a JSON object with: 1. 'summary' (overall health text), 2. 'gaps' (array of objects with EmployeeEmail, SkillName, RequiredLevel, ActualLevel, GapSeverity, and a short TrainingRecommendation), 3. 'scores' (array of objects with EmployeeEmail and Score (representing the overall skill match score out of 100, where each required skill has equal weight and the score decreases as gap severity increases, e.g., No Gap = 100, Minor Gap = 75, Moderate Gap = 50, Critical Gap = 0)).";
