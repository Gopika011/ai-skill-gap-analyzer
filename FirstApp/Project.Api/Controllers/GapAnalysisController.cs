...
public class GapAnalysisController : ControllerBase
{
    ...
    public async Task<IActionResult> RunAnalysis(int projectId)
    {
        ...
        var assignedUserIds = _context.ProjectEmployees
            .Where(pe => pe.ProjectId == projectId)
            .Select(pe => pe.UserId)
            .ToList();

        // 3. Fetch skills profile for those assigned employees (using Navigation Property User)
        var employeeSkills = _context.EmployeeSkill
            .Where(es => assignedUserIds.Contains(es.UserId))
            .Include(es => es.Skill)
            .Include(es => es.User)
            .Select(es => new
            {
                EmployeeEmail = es.User!.Email,
